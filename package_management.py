...
def activate_data_package(name, user_id):
    ...
    if not check_data_package_already_activated(user_id):
        ...
        return "You have already activated a data package. Please opt to change your package instead."
...
def activate_voice_package(name, user_id):
    ...
    if not check_voice_package_already_activated(user_id):
        if db_write(
                """INSERT INTO user_voice_package (package_id, user_id, package_name, activated_date) 
                VALUES (%s, %s, %s, %s)""",
...
def change_voice_package(name, user_id):
    ...
    if check_voice_package_already_activated(user_id):
        if db_write(
                """UPDATE user_voice_package 
                SET package_id=%s, package_name=%s, minutes_used=%s, activated_date=%s 
                WHERE user_id=%s""",
                (package_id, name, 0, activated_date, user_id)):
            remaining_minutes = get_voice_package(name)["minutes"]
...
def get_data_usage_data(user_id):
    usage_data = db_read(
        """SELECT package_name, data_used, activated_date, data, valid_period 
        FROM user_data_package, data_packages 
        WHERE user_id = %s AND user_data_package.package_id = data_packages.id""",
        (user_id,))
    ...
    if usage_data:
        ...
        if check_data_package_already_activated(user_id):
            return "Usage data retrieval failed. Please contact system administrator"
        else:
            return "You have no currently activated data packages"
...
def get_voice_usage_data(user_id):
    usage_data = db_read(
        """SELECT package_name, minutes_used, activated_date, minutes, valid_period 
        FROM user_voice_package, voice_packages 
        WHERE user_id = %s AND user_voice_package.package_id = voice_packages.id""",
        (user_id,))
