...
def get_data_usage_data(user_id):
    usage_data = db_read("""SELECT package_name, data_used, activated_date, data, valid_period FROM user_data_package, data_packages WHERE user_id = %s AND user_data_package.package_id = data_packages.id""", (user_id,))

    if usage_data:
        current_date = datetime.datetime.now()
        activated_date = usage_data[0]["activated_date"]
        valid_period = usage_data[0]["valid_period"]
        remaining_days = str(int(valid_period-((current_date-activated_date).days))-1) + " days "+ str(24-(current_date-activated_date).seconds//3600)+ " hours "+ str(60-((current_date-activated_date).seconds//60)%60)+ " minutes"  
...
def get_voice_usage_data(user_id):
    usage_data = db_read("""SELECT package_name, minutes_used, activated_date, minutes, valid_period FROM user_voice_package, voice_packages WHERE user_id = %s AND user_voice_package.package_id = voice_packages.id""", (user_id,))
    ...
    if usage_data:
        ...
        usage = "Active voice package: "+ usage_data[0]["package_name"] +"\n" +\
                "Remaining talktime: "+ str(int((usage_data[0]["minutes"] - usage_data[0]["minutes_used"])))+ " minutes\n"+\
                "Remaining days: "+ remaining_days
        return usage
    else:
        if check_voice_package_already_activated(user_id):
