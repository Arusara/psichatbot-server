from extract_info import ne_low_signal_location, ne_change_package, ne_package, ne_name, ne_data_package, \
    ne_voice_package

from package_management import check_data_package_name, get_data_packages, \
    get_data_package, activate_data_package, check_voice_package_name, get_voice_packages, get_voice_package, \
    activate_voice_package, \
...
def no_signal(tree, user_id, user_context):
    ...
    if location:
        ...
        if report_no_signal(user_id, location):
            return "We will look into the loss of signal in " + location + \
                   ". Thank you for staying with our network.", user_context
        else:
...
def low_signal(tree, user_id, user_context):
    ...
    if location:
        ...
        if report_low_signal(user_id, location):
            return "We will look into the weak signal in " + location + \
                   ". Thank you for staying with our network.", user_context
        else:
...
def change_voice_package_function(tree, user_context, user_id):
    ...
    for i in voice_packages_list:
        voice_packages_string += i["name"] + "-(" + str(i["minutes"]) + "minutes, " + str(i["valid_period"]).split(".")[
            0] + "days)\n"
    if package_name:
...
def get_voice_package_info():
    ...
    voice_packages_string = ""
    for i in voice_packages_list:
        voice_packages_string += i["name"] + "-(" + str(i["minutes"]).split(".")[0] + "minutes, " + \
                                 str(i["valid_period"]).split(".")[0] + "days, Rs. " + str(i["price"]) + " )\n"
...
def get_package_details(tree, user_context):
    ...
    if package_name:
        if check_data_package_name(package_name):
            ...
            package_details = i["name"] + "\nAvailable talktime:" + str(i["minutes"]).split(".")[
